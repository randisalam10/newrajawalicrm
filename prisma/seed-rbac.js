...
// prisma/seed-rbac.ts
var seed_rbac_exports = {};
__export(seed_rbac_exports, {
  PERMISSIONS: () => PERMISSIONS,
  ROLES: () => ROLES
});
module.exports = __toCommonJS(seed_rbac_exports);
...
var ROLES = [
  ...
  },
  {
    ...
    permissions: [
      ...
      "BILLING_APPROVE",
      "MATERIAL_SEMEN_VIEW",
      "MATERIAL_SEMEN_CREATE",
      "MATERIAL_SEMEN_EDIT",
      "MATERIAL_AGREGAT_VIEW",
      "MATERIAL_AGREGAT_CREATE",
      "MATERIAL_AGREGAT_EDIT",
      "MATERIAL_USAGE_VIEW",
      "PLANNING_VIEW",
      "PLANNING_CREATE",
      "PLANNING_EDIT",
...
async function main() {
  ...
  for (const r of ROLES) {
    ...
    const targetPermCodes = r.allPermissions ? allDbPermissions.map((p) => p.code) : r.permissions || [];
    const targetPermIds = allDbPermissions.filter((p) => targetPermCodes.includes(p.code)).map((p) => p.id);
